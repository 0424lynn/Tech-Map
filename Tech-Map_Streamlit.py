...
# Basic
# ======================
st.set_page_config(page_title="Tech Map", layout="wide", initial_sidebar_state="expanded")
...
# ======================
# Data Source panel (DEFINE ONLY; will be rendered at BOTTOM)
# ======================
def render_data_source_panel():
    st.sidebar.markdown("---")
    with st.sidebar.expander("📁 Data Source (folder)", expanded=True):
        cur_dir = st.text_input(
            "Data folder path",
            value=st.session_state.get("data_dir_path", DATA_DIR_DEFAULT),
            key="data_dir_path_input"
        )
        if cur_dir != st.session_state.get("data_dir_path", DATA_DIR_DEFAULT):
            st.session_state.data_dir_path = cur_dir
        os.makedirs(st.session_state.data_dir_path, exist_ok=True)
        st.caption(f"Current: {st.session_state.data_dir_path}")

        st.markdown("**Technicians**")
        tech_files = _list_tech_files()
        if tech_files:
            pick = st.selectbox("Pick saved tech file", tech_files, index=0, key="pick_tech_file_always")
            if st.button("Load selected (tech)", key="btn_load_tech_always"):
                try:
        ...
        new_file = st.file_uploader("Upload tech data (save into folder)",
                                    type=['csv','xlsx','xls'], key="uploader_tech_always")
        ...
        if cfiles:
            cpick = st.selectbox("Pick saved customer file", cfiles, index=0, key="pick_cust_file_always")
            if st.button("Load selected (customers)", key="btn_load_cust_always"):
                try:
        ...
        cust_file = st.file_uploader("Upload customers (save into folder)",
                                     type=['csv','xlsx','xls'], key="uploader_cust_always")
...
# Flag
HAS_DATA = st.session_state.get("df", None) is not None

# =====================================================================
# ======================= MAIN (only if HAS_DATA) ======================
# =====================================================================
if HAS_DATA:

    # ======================
    # Address helpers & geocoder pieces reused later
    # ======================
    ...
    if not lat_col or not lon_col:
        ...
        only_dup = st.session_state.get("only_dup_addr_v3", False)
        if only_dup:
...
# =====================================================================
# =================== END MAIN — bottom Data Source ====================
# =====================================================================

# ---- 仅在“最底部”渲染 Data Source 面板 ----
render_data_source_panel()

# 如果还没加载到技师数据，这里再给提示并终止（保证 Data Source 在底部已渲染）
if not HAS_DATA:
    st.warning("No technician dataset loaded yet — use the bottom of the left sidebar (📁 Data Source) to upload or pick a file.")
    st.stop()
